...
リポジトリの `feat-deck-generator` ディレクトリへ移動し、プロジェクトの依存関係を
インストールします。デンドログラム生成には `matplotlib` が必要で、`requirements.txt`
に含まれています。

```bash
cd /path/to/pokemon-hackason/feat-deck-generator
python3 -m venv .venv
