...
class HiddenZones(Protocol):
    """search_beginへ渡す非公開zone sampleの最小interface。"""
...
def create_node(
    ...
    """探索状態からMCTSノードを作り、手番側に対応するモデルで評価する。"""
...
def mcts_agent(
    ...
    hidden_zones: HiddenZones | None = None,
    rng: random.Random | None = None,
    opponent_model: MyModel | None = None,
) -> tuple[list[int], LearnSample | None]:
    """指定した非公開zoneのMCTSで手を選び、root学習sampleを返す。

    hidden_zonesを渡すと観測と整合した決定化を使用する。省略時は旧来互換の
    簡易sampleを使う。rngを明示すればfallbackも再現可能になる。
    """
    ...
    if hidden_zones is None:
        search_arguments = {
            "your_deck": rng.sample(
                your_deck,
                min(len(your_deck), state.players[your_index].deckCount),
            ),
            "your_prize": rng.sample(
                your_deck,
                min(len(your_deck), len(state.players[your_index].prize)),
            ),
        ...
        search_arguments = {
            "your_deck": hidden_zones.your_deck,
            "your_prize": hidden_zones.your_prize,
            "opponent_deck": hidden_zones.opponent_deck,
            "opponent_prize": hidden_zones.opponent_prize,
            "opponent_hand": hidden_zones.opponent_hand,
            "opponent_active": hidden_zones.opponent_active,
        }
    ...
    try:
        ...
        if not root.children:
            # NNや探索が候補を作れなくても、合法候補の先頭を安定して返す。
            return list(range(min(len(obs.select.option), obs.select.maxCount))), sample
