...
def _run_determinization(
    ...
    """具体化した山札・サイド・手札で通常の完全情報MCTSを1回行う。"""
...
def mcts_agent(
    ...
    """複数の非公開領域仮説でroot訪問数を集約し、MCTSの手を選ぶ。"""

    obs = to_observation_class(obs_dict)
    if obs.select is None:
        return your_deck, None

    evaluation_opponent_model = opponent_model or model
    ...
    selected_key = max(
        visits,
        key=lambda key: (visits[key], priors.get(key, 0.0)),
    )
