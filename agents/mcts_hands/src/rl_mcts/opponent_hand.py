...
DEFAULT_DECK_DATABASE = Path(__file__).with_name("deck_candidates_by_wins.jsonl")
...
class HandModelPredictor:
    def __init__(
        ...
        if enabled:
            if not path.exists():
                raise FileNotFoundError(f"hand prediction model was not found: {path}")
...
class OpponentBelief:
    ...
    def __init__(
        ...
        self.records = load_candidates(database_path or DEFAULT_DECK_DATABASE)
        self.tracker = OpponentHandTracker()
        self.hand_predictor = HandModelPredictor(
            hand_model_path or DEFAULT_HAND_MODEL,
            enabled=use_hand_model,
        )
        self.basic_pokemon = {
